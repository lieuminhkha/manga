...
const Register = ({
  ...
  return (
    <div
      className={`form-register fixed z-20 -translate-x-1/2 -translate-y-1/2  bg-white 
    rounded-md overflow-hidden shadow-3 cursor-pointer top-1/2 ${
      isRegisterFormVisible ? "left-1/2 opacity-100" : "left-1/4 opacity-50"
    }`}
    >
      <div>
        <FontAwesomeIcon
          icon={faLeftLong}
          className="text-32 float-left pl-2"
          onClick={(e) => {
        ...
        <FontAwesomeIcon
          ...
          onClick={(e) => setShowModalRegister(false)}
        />
      </div>
      <form className="pt-8 pb-3 pr-40 pl-3">
